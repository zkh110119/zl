<template>
  <el-container>
    <el-main>
      <el-row :gutter="20">
        <el-col :span="12" v-for="chart of gaComp" :key="chart">
...
export default {
  ...
  data() {
    return {
      ga: null,
      gaComp: ["chart1", "chart2", "chart3", "chart4"],
      times: {
        chart1: 0,
        chart2: 0,
        chart3: 0,
  ...
  created() {
    ...
    console.log(
      `图表区块初始顺序为：`,
      this.gaComp.map((item) => {
        return item;
      })
    );
    const gaSort = this.ga.getAnchor();
    console.log(
        `%c获取图表区块的埋点统计数据为：`,
        `color:orange;font-weight:700;`,
        gaSort
      );
    this.gaComp = [...this.gaComp].sort((a, b) => {
      return gaSort[b] - gaSort[a];
    });
    console.log(
        `%c图表区块埋点值进行重排结果为：`,
  ...
  mounted() {
    this.gaComp.forEach((item) => {
      this.initChart(item);
    });
  },
  beforeRouteLeave(to, from, next) {
    const keyArr = Object.keys(this.times);
    keyArr.sort((a, b) => {
      return this.times[b] - this.times[a];
    });
    keyArr.forEach((key, index) => {
      this.ga.add(key, index + 1);
    });
    next();
  },
  methods: {
    initChart(eleId) {
      ...
      const chart = this.$echarts.init(document.getElementById(eleId));
      const options = {
        chart1: {
      ...
      chart.setOption(options[eleId]);
      chart.on("globalout", () => {
        if (this.timeStart[eleId + "Start"] === 0) {
          this.timeStart[eleId + "Start"] = +new Date();
        } else {
          this.times[eleId] += +new Date() - this.timeStart[eleId + "Start"];
          this.timeStart[eleId + "Start"] = 0;
        }
      });
    },
  },
